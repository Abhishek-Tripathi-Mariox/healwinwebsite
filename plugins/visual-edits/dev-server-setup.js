...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all healwin.in subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*healwin\.in$/)) {
        return true;
      }
      ...
      try {
        ...
        for (const change of variableEditChanges) {
          ...
          if (result.success) {
            ...
            try {
              execSync(
                `git -c user.name="visual-edit" -c user.email="support@healwin.in" add "${result.file}"`,
              );
              execSync(
                `git -c user.name="visual-edit" -c user.email="support@healwin.in" commit -m "visual_edit_variable_${timestamp}"`,
              );
            } catch (gitError) {
              console.error(
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(
              `git -c user.name="visual-edit" -c user.email="support@healwin.in" add "${targetFile}"`,
            );
            execSync(
              `git -c user.name="visual-edit" -c user.email="support@healwin.in" commit -m "visual_edit_${timestamp}"`,
            );
          } catch (gitError) {
